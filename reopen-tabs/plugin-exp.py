...
RELOADER_STATE_READY     = "reloading"
...
#                dlg_save = gtk.MessageDialog(None, gtk.DIALOG_MODAL, gtk.MESSAGE_QUESTION, gtk.BUTTONS_YES_NO, "Reopen tabs on next run?")
	...
	active = self._config.get("common", "active_document")
        ...
        if len(docs) > 0:
            ...
            if doc.is_untitled():
                # Get first document in list
                first = docs[0]
                docs = docs[1:]
            
                tab = gedit.tab_get_from_document(doc)
		uri = self._config.get("documents", first)
                # Load the first document in an empty tab
#                doc.load(self._config.get("documents", first), gedit.encoding_get_current(), 0, False)
		
                # Check if document was active
                if first == active:
                    self._doc_signals[uri] = tab.get_document().connect("loaded", self._on_active_doc_loaded, uri)
                else:
                    self._doc_signals[uri] = tab.get_document().connect("loaded", self._on_doc_loaded, uri)
            
            # Process the rest documents
            for d in docs:
                tab = window.create_tab(False)#_from_uri(self._config.get("documents", d), gedit.encoding_get_current(), 0, True, False)
                tab.get_document().connect("loaded", self._on_doc_loaded, self._config.get("documents", d))
		
                # Check if document was active
                if d == active:
                    self._doc_signals[uri] = tab.get_document().connect("loaded", self._on_active_doc_loaded, self._config.get("documents", d))
                else:
                    self._doc_signals[uri] = tab.get_document().connect("loaded", self._on_doc_loaded, self._config.get("documents", d))
                

            # Connect handler that switches saved active document tab
#	    if self._active_tab:
#	        self._active_tab.get_document().connect("loaded", self._on_active_doc_loaded)

    def _on_doc_loaded(self, doc, arg, uri): # Switches to saved active document tab
        # Disconnect this signal
        doc.disconnect(self._doc_signals[uri])
        
        # Load document
        doc.load(uri, gedit.encoding_get_current(), 0, False)
        print uri
                
    def _on_active_doc_loaded(self, doc, arg, uri): # Switches to saved active document tab
        # Disconnect this signal
        doc.disconnect(self._doc_signals[uri])
        ...
        app  = gedit.app_get_default()
        win  = app.get_active_window()
        win.set_active_tab(gedit.tab_get_from_document(doc))
